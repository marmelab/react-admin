...
import { push as pushAction, goBack as goBackAction } from 'react-router-redux';
...
export class List extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.resource !== this.props.resource
         ...
         || nextProps.query.export !== this.props.query.export
         || nextProps.query.filter !== this.props.query.filter) {
            this.updateData(this.getQuery(nextProps));
        }
    ...
    getQuery(props = this.props) {
        return (Object.keys(props.query).length > 0) ? props.query : { ...props.params };
    }
    ...
    render() {
        const { filter, resource, hasCreate, title, data, ids, total, children, isLoading, export: Export } = this.props;
        const query = this.getQuery();
        ...
        return (
            <Card style={{ margin: '2em', opacity: isLoading ? 0.8 : 1 }}>
                <CardActions style={{ zIndex: 2, display: 'inline-block', float: 'right' }}>
                    ...
                    {Export && (
                        <FlatButton primary label="Export" onClick={this.exportClicked} icon={<DownloadIcon />} />
                    )}
