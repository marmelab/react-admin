...
localStorage.setItem('full_name', 'Jane Doe');

const fakeFetchUser = async ({ email }: { email: string }) => {
...
export const authProvider: AuthProvider = {
    login: async ({ email, redirectTo }) => {
        const user = await fakeFetchUser({ email });
        if (user) {
    ...
    getIdentity: () =>
        Promise.resolve({
            id: 0,
            fullName: localStorage.getItem('full_name') || 'Jane Doe',
