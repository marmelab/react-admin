...
localStorage.setItem('user', JSON.stringify({ id: 0, full_name: 'Jane Doe' }));
...
export const authProvider: AuthProvider = {
    login: async ({ email, redirectTo }) => {
        ...
        localStorage.setItem('user', JSON.stringify({ full_name: 'Jane Doe' }));
        return Promise.resolve(redirectTo ? { redirectTo } : undefined);
    },
    logout: () => {
    ...
    getIdentity: () => {
        ...
        const fullName = user ? JSON.parse(user).full_name : 'Jane Doe';
        const id = user ? JSON.parse(user).id : 0;
        return Promise.resolve({
            id,
            fullName,
