...
describe('stripTags', () => {
    ...
    it('should strip HTML embedded tags', () => {
        assert.equal(removeTags('<marquee><a href="http://www.zombo.com">Zombo</a></marquee>'), 'Zombo');
    });

    it('should strip HTML tags even if they are malformed', () => {
        assert.equal(removeTags('<p>All our base is belong to us.<p>'), 'All our base is belong to us.');
    });
});
...
describe('<RichTextField />', () => {
    ...
    it('should strip HTML tags if stripTags is set to true', () => {
        ...
        assert.equal(html(wrapper), '<span class="MuiTypography-root-1 MuiTypography-body1-10">Hello world!</span>');
    });
    ...
    it('should use custom className', () =>
        assert.deepEqual(
            shallow(<RichTextField record={{ foo: true }} source="body" className="foo" />).prop('className'),
            'foo'
        ));
});
