...
describe('<Pagination />', () => {
    ...
    describe('Pagination buttons', () => {
        it('should display a next button when there are more results', () => {
            const { queryByText } = render(<Pagination {...defaultProps} rowsPerPage={1} total={2} page={1} />);
            expect(queryByText('ra.navigation.next')).not.toBeNull();
        });
        it('should not display a next button when there are no more results', () => {
            const { queryByText } = render(<Pagination {...defaultProps} rowsPerPage={1} total={2} page={2} />);
    ...
    describe('mobile', () => {
        it('should not render a rowsPerPage choice', () => {
            const { queryByText } = render(<Pagination {...defaultProps} page={2} perPage={5} total={15} width="xs" />);
    ...
    describe('desktop', () => {
        it('should render rowsPerPage choice', () => {
            const { queryByText } = render(<Pagination {...defaultProps} page={2} perPage={5} total={15} width="md" />);
