...
describe("<Pagination />", () => {
  it("should display a pagination limit when there is no result", () => {
    ...
    expect(wrapper.find("pure(translate(PaginationLimit))")).toHaveLength(1);
  });
  ...
  it("should not display a pagination limit when there are results", () => {
    ...
    expect(wrapper.find("pure(translate(PaginationLimit))")).toHaveLength(0);
  });
  ...
  describe("mobile", () => {
    it("should render a <TablePagination> without rowsPerPage choice", () => {
      const wrapper = shallow(
        <Pagination page={2} perPage={5} total={15} translate={x => x} />
      )
        .shallow()
        .shallow()
        .setProps({ width: "xs" })
