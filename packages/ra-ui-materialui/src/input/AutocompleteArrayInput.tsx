import React, { useCallback, useEffect, useRef, FunctionComponent, useMemo, isValidElement } from 'react';
 ...
 */
const AutocompleteArrayInput: FunctionComponent<InputProps<TextFieldProps & Options> & DownshiftProps<any>> = (
	props
) => {
	const {
		allowDuplicates,
...
/>
	...
	const { id, input, isRequired, meta: { touched, error } } = useInput({
		format,
		id: idOverride,
		input: inputOverride,
		meta: metaOverride,
	...
	const selectedItems = useMemo(() => values.map(getSuggestionFromValue), [ ...values, getSuggestionFromValue ]);
						...
						<TextField
							...
							label={
								<FieldTitle
									...
									resource={resource}
									isRequired={
										typeof isRequiredOverride !== 'undefined' ? isRequiredOverride : isRequired
									}
								/>
							}
...
const useStyles = makeStyles(
	(theme) => {
		const chipBackgroundColor =
			theme.palette.type === 'light' ? 'rgba(0, 0, 0, 0.09)' : 'rgba(255, 255, 255, 0.09)';
