...
describe('<DateInput />', () => {
    ...
    it('should call `input.onChange` method when changed', () => {
        ...
        const { getByLabelText } = render(<DateInput {...defaultProps} input={{ onChange }} />);
    ...
    describe('error message', () => {
        it('should not be displayed if field is pristine', () => {
            const { container } = render(<DateInput {...defaultProps} meta={{ touched: false }} />);
            expect(container.querySelector('p')).toBeNull();
        });

        it('should not be displayed if field has been touched but is valid', () => {
            const { container } = render(<DateInput {...defaultProps} meta={{ touched: true, error: false }} />);
            expect(container.querySelector('p')).toBeNull();
        });

        it('should be displayed if field has been touched and is invalid', () => {
            const { container, queryByText } = render(
                <DateInput {...defaultProps} meta={{ touched: true, error: 'Required field.' }} />
            );
