...
describe('Linking to a record', () => {
    ...
    it('should generate valid show links if requested', () => {
        ...
        assert.equal(linkToRecord('books', '/books/13', 'show'), 'books/%2Fbooks%2F13/show');
        assert.equal(linkToRecord('blogs', 'https://dunglas.fr', 'show'), 'blogs/https%3A%2F%2Fdunglas.fr/show');
    });
});
