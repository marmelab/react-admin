...
Use the `queryOptions` prop to pass options to the `dataProvider.getList()` query that fetches the possible choices, as well as the `dataProvider.getMany()` query that retrieves the currently selected choices.
