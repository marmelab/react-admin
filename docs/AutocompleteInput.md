...
| `optionText`               | Optional | `string` &#124; `Function` &#124; `Component` | `recordRepresentation` (if inside a ReferenceInput) &#124; `undefined` | Field name of record to display in the suggestion item or function which accepts the correct record as argument (`(record)=> {string}`)           |
...
To override the style of all instances of `<AutocompleteInput>` using the [MUI style overrides](https://mui.com/customization/globals/#css), use the `RaAutocompleteInput` key.
