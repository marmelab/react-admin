...
The `FileInput` component accepts an `options` prop into which you can pass all the [react-dropzone properties](https://github.com/okonet/react-dropzone#features). For instance, if you need to limit the maximum file size to 2 MB, just add the {% raw %}`options={{ maxSize: 2000000 }}`{% endraw %} DropZone attribute to your `<FileInput />` field.
If you need to upload several files at once, you must add the `multiple` DropZone attribute to your `<FileInput />` field.
