...
The component you pass to `addButton` and `removeButton` will be cloned and passed a `onClick` prop. The component you pass to `reOrderButtons` will be cloned with the following props:

-   `index`: the index of the item.
-   `max`: the total number of items. Useful to disable a button which moves an item down.
-   `onReorder`: a function accepting two parameters: a source index and a destination index and which will move the item at the `source` index to the `destination` index.
...
**Note**: You **must** add a `<Resource>` for the reference resource - react-admin needs it to fetch the reference data. You _can_ omit the `list` prop in this reference if you want to hide it in the sidebar menu.
