...
| [`emptyText`](#emptytext)     | Optional | `string` | `ReactNode`    | ''       | Defines a text or element to be shown when a field has no value (not supported in array fields)                                                                    |
...
const PostList = () => (
    <List>
        <Datagrid>
            <TextField source="title" />
            {/* emptyText can be a string */}
            <TextField source="author" emptyText="missing data" />
            {/* you can use React Node as emptyText */}
            <TextField source="email" emptyText={<em>missing data</em>} />
        </Datagrid>
    </List>
);
```
