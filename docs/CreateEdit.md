...
The following example shows how to make use of scrollable `<Tabs>`. Pass `variant` and `scrollButtons` props to `<TabbedFormTabs>` and pass that as the `tabs` prop to `<TabbedForm>`.
