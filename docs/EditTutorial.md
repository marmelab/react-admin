...
React-hook-form requires developers to create a form `control`, and to wrap each input with a `<Controller>` tag. We can put this logic into a custom Form component to avoid repeating it for every form. This is what [the `<SimpleForm>` component](./SimpleForm.md) does:
