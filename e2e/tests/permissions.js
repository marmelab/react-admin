...
describe('Permissions', () => {
  const CreatePage = createPageFactory('http://localhost:8083/#/users/create')(
    driver
  );
  ...
  describe('Resources', () => {
    it('hides protected resources depending on permissions', async () => {
      ...
      assert.deepEqual(await ListPage.getResources(), ['Posts', 'Comments']);
    });
  ...
  describe('shows protected data depending on permissions', () => {
    ...
    it('in List page filters', async () => {
      await ListPage.navigate();
      assert.deepEqual(await ListPage.getAvailableFilters(), ['Name', 'Role']);
    });
