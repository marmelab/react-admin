...
describe('Edit Page', () => {
  ...
  const EditCommentPage = editPageFactory('http://localhost:8083/#/comments/5')(
    driver
  );
  ...
  describe('TabbedForm', () => {
    ...
    it('should allow to update elements', async () => {
      ...
      assert.equal(await EditPostPage.getInputValue('title'), 'Lorem Ipsum');
    ...
    it('should keep DateInput value after opening datapicker', async () => {
      await EditPostPage.gotoTab(3);
      const valueBeforeClick = await EditPostPage.getInputValue('published_at');
