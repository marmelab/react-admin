...
export default url => driver => ({
    ...
    return driver
      ...
      .then(
        () =>
          continued
            ? driver.wait(
                until.stalenessOf(driver.findElement(this.elements.appLoader))
              )
            : true
      )
  ...
  getAvailableFilters() {
    ...
    return driver
      ...
      .then(filters => Promise.all(filters.map(filter => filter.getText())));
  },
  ...
  async setFilterValue(name, value, clearPreviousValue = true) {
    const filterField = await driver.findElement(this.elements.filter(name));
  ...
  async showFilter(name) {
    ...
    await driver.wait(until.elementLocated(this.elements.filterMenuItem(name)));
  ...
  applyUpdateBulkAction() {
    return driver
      ...
      .then(() =>
        driver.wait(
          until.elementLocated(this.elements.customBulkActionsButtonMenuItem),
          500
        )
  ...
  applyDeleteBulkAction() {
    return driver
      ...
      .then(() =>
        driver.wait(
          until.elementLocated(this.elements.deleteBulkActionsButtonMenuItem),
          500
        )
      ) // wait until animations end
      .then(() =>
